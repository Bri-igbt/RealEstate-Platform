    ...
    try {
        ...
        if (req.files && req.files.length > 0) {
            for (let file of req.files) {
                const result = await uploadCloudinary(file.buffer, "properties");
        ...
        console.error("ADD PROPERTY_ERROR:", error);
...
export const getMyProperty = async (req, res) => {
    try {
        ...
        console.error("GETTING MY PROPERTY_ERROR:", error);
        res.status(500).json({
            success: false,
            message: error.message || "Internal server error while getting my property"
        });
    }
};
...
export const updateProperty = async (req, res) => {
    try {
        ...
        console.error("UPDATE PROPERTY_ERROR:", error);
        const statusCode = error.name === "ValidationError" ? 400 : 500;
        res.status(statusCode).json({
            success: false,
